...
class Paypalexpress extends Model {
	...
	public function getMethod($address, $total) {
		...
		$query = $this->db->query("
			...
			WHERE `geo_zone_id` = '" . (int)$this->config->get('paypalexpress_geo_zone_id') . "' 
			AND `country_id` = '" . (int)$address['country_id'] . "' 
			AND (`zone_id` = '" . (int)$address['zone_id'] . "' OR `zone_id` = '0')");

		if ($this->config->get('paypalexpress_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('paypalexpress_geo_zone_id')) {
			$status = true;
		...
		if ($status) {
			$method_data = array(
				...
				'terms'      => '',
				'sort_order' => $this->config->get('paypalexpress_sort_order')
			);
		}
	...
	public function paymentRequestInfo() {
		...
		$data['PAYMENTREQUEST_0_PAYMENTACTION'] = $this->config->get('paypalexpress_method');
		...
		foreach ($total_data as $total_row) {
			if (!in_array($total_row['code'], array('total', 'subtotal'))) {
	...
	public function totalCaptured($paypal_order_id) {
		$qry = $this->db->query("
			...
			AND (`payment_status` = 'Partially-Refunded' OR `payment_status` = 'Completed' OR `payment_status` = 'Pending') 
