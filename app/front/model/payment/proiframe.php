...
class Proiframe extends Model {
	public function getMethod($address, $total) {
		...
		$query = $this->db->query("SELECT * FROM {$this->db->prefix}zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('proiframe_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

		if ($this->config->get('proiframe_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('proiframe_geo_zone_id')) {
			$status = true;
		...
		if ($status) {  
			$method_data = array(
				...
				'sort_order' => $this->config->get('proiframe_sort_order')
			);
		}
