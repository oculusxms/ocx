...
class Loworderfee extends Model {
    public function getTotal(&$total_data, &$total, &$taxes) {
        if ($this->cart->getSubTotal() && ($this->cart->getSubTotal() < $this->config->get('loworderfee_total'))) {
            ...
            $total_data[] = array('code' => 'loworderfee', 'title' => $this->language->get('text_loworderfee'), 'text' => $this->currency->format($this->config->get('loworderfee_fee')), 'value' => $this->config->get('loworderfee_fee'), 'sort_order' => $this->config->get('loworderfee_sort_order'));
