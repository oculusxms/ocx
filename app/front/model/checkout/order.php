...
class Order extends Model {
    ...
    public function confirm($order_id, $order_status_id, $comment = '', $notify = true) {
        ...
        if ($order_info/* && !$order_info['order_status_id']*/) {
                ...
                if ($event_query->row['event_id'] > 0) {
                    ...
                    if (!empty($rosters->row['roster'])):
                        foreach (unserialize($rosters->row['roster']) as $row):
                            $roster[] = array('attendee_id' => $row['attendee_id'], 'date_added' => $row['date_added']);
                        endforeach;
                        
                        $roster[] = array('attendee_id' => $order_info['customer_id'], 'date_added' => time());
                    else:
                        $roster[] = array('attendee_id' => $order_info['customer_id'], 'date_added' => time());
                    endif;
            ...
            if ($product_reward && ($order_status_id == $this->config->get('config_order_status_id'))):
                $description = sprintf($this->language->get('lang_text_new_reward'), $product_reward, $order_id);
            ...
            if (isset($order_info['affiliate_id']) && isset($order_info['commission'])):
				...
				$description = sprintf($this->language->get('lang_text_new_commission'), $commission, $order_id);
            ...
            if (isset($order_info['customer_group_id'])) {
                ...
                $customer_group_info = $this->model_account_customergroup->getCustomerGroup($order_info['customer_group_id']);
            }
    ...
    //         $template->data['table_quantity'] = true;
    ...
    //         $template->data['config_stock_display'] = $this->config->get('config_stock_display');
    ...
    //         $template->data['has_hangout'] = $has_hangout;
    ...
    //         $html = $template->fetch('mail/order_customer');
    ...
    //         unset($html);
     ...
     //            $template->data['order_link'] = $this->app['https.server'] . ADMIN_FASCADE . '/index.php?route=sale/order/info&order_id=' . $order_id;
                
     //            $html = $template->fetch('mail/order_admin');
