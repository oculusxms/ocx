	...
	public function confirm($order_id, $order_status_id, $comment = '', $notify = true) {
		...
		if ($order_info /* && !$order_info['order_status_id']*/) {
			...
			if ($product_reward && ($order_status_id == $this->config->get('config_order_status_id'))):
				$description = sprintf($language->get('text_new_reward'), $product_reward, $order_id);
			...
			foreach ($order_product_query->rows as $product) {
				...
				$template->data['products'][] = array(
					...
					'url'				=> $order_info['store_url'] . 'index.php?route=product/product&product_id=' . $product['product_id'],
					'url_admin'			=> $this->app['https.server'] . ADMIN_FASCADE . '/index.php?route=catalog/product/update&product_id=' . $product['product_id'],
					'name'     			=> $product['name'],
					'model'    			=> $product['model'],
					'option'   			=> $option_data,
