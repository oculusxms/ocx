        ...
        if ($this->config->get('config_home_page')):
            ...
            foreach ($posts as $post):
                if ($post['image']):
                    ...
                    $data['posts'][] = array('post_id' => $post['post_id'], 'author_name' => $post['author_name'], 'thumb' => $image, 'name' => $post['name'], 'short' => utf8_substr(strip_tags(html_entity_decode($post['description'], ENT_QUOTES, 'UTF-8')), 0, 450) . '..', 'blurb' => utf8_substr(strip_tags(html_entity_decode($post['description'], ENT_QUOTES, 'UTF-8')), 0, 200) . '..', 'rating' => $rating, 'views' => sprintf($this->language->get('text_views'), (int)$post['viewed']), 'comments' => sprintf($comment_text, (int)$post['comments']), 'href' => $this->url->link('content/post', 'post_id=' . $post['post_id']), 'comments_href' => $this->url->link('content/post', 'post_id=' . $post['post_id'] . '&to_comments=1'), 'author_href' => $this->url->link('content/search', '&filter_author_id=' . $post['author_id']), 'date_added' => date($this->language->get('post_date'), strtotime($post['date_added'])), 'categories' => $posted_in_categories);
                endforeach;
