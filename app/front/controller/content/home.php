	...
	public function index() {
		...
		if ($this->config->get('config_home_page')):
			...
			foreach ($posts as $post):
				if ($post['image']):
					$image = IMAGE_URL . $post['image']; // remove resizing and allow img-responsive to handle sizing
				else:
					$image = '';
				endif;		
				...
				$data['posts'][] = array(
					...
					'comments_href'	=> $this->url->link('content/post', 'post_id=' . $post['post_id'] . '&to_comments=1'),
					'author_href' 	=> $this->url->link('content/search', '&filter_author_id=' . $post['author_id']),
					'date_added' 	=> date($this->language->get('post_date'), strtotime($post['date_added'])),
			...
			$data['pagination'] = $this->theme->paginate(
				...
				$this->url->link('content/home', $url . '&page={page}')
			);
