	...
	public function index() { 
		...
		if ($category_info) {
			...
			if ($category_info['image']) {
				$data['thumb'] = IMAGE_URL . $category_info['image']; // remove resizing and allow img-responsive to handle sizing
			...
			foreach ($results as $result) {
				if ($result['image']) {
					$image = IMAGE_URL . $result['image']; // remove resizing and allow img-responsive to handle sizing
				} else {
					$image = '';
				}		
				...
				$data['posts'][] = array(
					...
					'comments_href'	=> $this->url->link('content/post', 'post_id=' . $result['post_id'] . '&to_comments=1'),
					'author_href' 	=> $this->url->link('content/search', '&filter_author_id=' . $result['author_id']),
					'date_added' 	=> date($this->language->get('post_date'), strtotime($result['date_added'])),
			...
			$data['pagination'] = $this->theme->paginate(
				...
				$this->url->link('content/category', 'bpath=' . $this->request->get['bpath'] . $url . '&page={page}')
			);
