		...
		if ($results) {
			foreach ($results as $result) {
				...
				$data['waitlists'][] = array(
					...
					'name'			=> html_entity_decode($event_info['name'], ENT_QUOTES, 'UTF-8'),
					'start_date'	=> date($this->language->get('date_format_short'), strtotime($event_info['date_time'])),
					'location'		=> nl2br($event_info['location']),
					'telephone'		=> $event_info['telephone'] ? $event_info['telephone'] : 'N/A',
					'remove'		=> $this->url->link('account/waitlist', 'event_id=' . $result['event_id'], 'SSL')
				);
			}
		}
