...
class Paypalpro extends Controller {
    public function index() {
        ...
        $this->theme->loadjs('javascript/payment/paypalpro', $data);
        
        $data = $this->theme->listen(__CLASS__, __FUNCTION__, $data);
        
        $data['javascript'] = $this->theme->controller('common/javascript');
        
        return $this->theme->view('payment/paypalpro', $data);
    }
    
    public function send() {
        if (!$this->config->get('paypalpro_transaction')) {
        ...
        $request.= '&USER=' . urlencode($this->config->get('paypalpro_username'));
        $request.= '&PWD=' . urlencode($this->config->get('paypalpro_password'));
        $request.= '&SIGNATURE=' . urlencode($this->config->get('paypalpro_signature'));
        ...
        if (!$this->config->get('paypalpro_test')) {
