...
class Paypalexpress extends Controller {
    public function index() {
        ...
        $data['button_continue_action'] = $this->url->link('payment/paypalexpress/checkout', '', 'SSL');
        ...
        $this->theme->loadjs('javascript/payment/paypalexpress', $data);
        
        $data = $this->theme->listen(__CLASS__, __FUNCTION__, $data);
        
        $data['javascript'] = $this->theme->controller('common/javascript');
        
        return $this->theme->view('payment/paypalexpress', $data);
    }
    ...
    public function express() {
        ...
        $data = array_merge($data, $this->model_payment_paypalexpress->paymentRequestInfo());
    ...
    public function expressConfirm() {
        $data = $this->theme->language('payment/paypalexpress');
        $data = $this->theme->language('checkout/cart', $data);
        ...
        $data['action'] = $this->url->link('payment/paypalexpress/expressConfirm', '', 'SSL');
            ...
            if (!empty($shipping_address)) {
                ...
                if (!empty($results)) {
                    ...
                    if (!empty($quote_data)) {
                        ...
                        $data['action_shipping'] = $this->url->link('payment/paypalexpress/shipping', '', 'SSL');
                    } else {
                        unset($this->session->data['shipping_methods']);
                        unset($this->session->data['shipping_method']);
        ...
        $this->theme->loadjs('javascript/payment/paypalexpress_confirm', $data);
        ...
        $this->response->setOutput($this->theme->view('payment/paypalexpress_confirm', $data));
    }
    ...
    public function expressComplete() {
        ...
        if ($redirect == '') {
            ...
            $result = $this->model_payment_paypalexpress->call($paypal_data);
            ...
            if ($result['ACK'] == 'Success') {
                ...
                switch ($result['PAYMENTINFO_0_PAYMENTSTATUS']) {
                    case 'Canceled_Reversal':
                        $order_status_id = $this->config->get('paypalexpress_canceled_reversal_status_id');
                        break;

                    case 'Completed':
                        $order_status_id = $this->config->get('paypalexpress_completed_status_id');
                        break;

                    case 'Denied':
                        $order_status_id = $this->config->get('paypalexpress_denied_status_id');
                        break;

                    case 'Expired':
                        $order_status_id = $this->config->get('paypalexpress_expired_status_id');
                        break;

                    case 'Failed':
                        $order_status_id = $this->config->get('paypalexpress_failed_status_id');
                        break;

                    case 'Pending':
                        $order_status_id = $this->config->get('paypalexpress_pending_status_id');
                        break;

                    case 'Processed':
                        $order_status_id = $this->config->get('paypalexpress_processed_status_id');
                        break;

                    case 'Refunded':
                        $order_status_id = $this->config->get('paypalexpress_refunded_status_id');
                        break;

                    case 'Reversed':
                        $order_status_id = $this->config->get('paypalexpress_reversed_status_id');
                        break;

                    case 'Voided':
                        $order_status_id = $this->config->get('paypalexpress_voided_status_id');
                        break;
                }
    ...
    public function ipn() {
        $this->theme->model('payment/paypalexpress');
        $this->theme->model('payment/expressipn');
        ...
        if ((string)$response == "VERIFIED") {
            ...
            if (isset($this->request->post['txn_id'])) {
                $transaction = $this->model_payment_paypalexpress->getTransactionRow($this->request->post['txn_id']);
            } else {
                $transaction = false;
            }
            ...
            if ($transaction) {
                ...
                if ($this->config->get('paypalexpress_debug') == 1) $this->log->write('Transaction exists');
                ...
                if ($transaction['payment_status'] != $this->request->post['payment_status']) {
                    $this->model_payment_expressipn->updateStatus($this->request->post['payment_status'], $transaction['transaction_id']);
                } elseif ($transaction['payment_status'] == 'Pending' && ($transaction['pending_reason'] != $this->request->post['pending_reason'])) {
                    $this->model_payment_expressipn->updatePending($this->request->post['pending_reason'], $transaction['transaction_id']);
                }
            } else {
                if ($this->config->get('paypalexpress_debug') == 1) $this->log->write('Transaction does not exist');
                
                $parent_transaction = false;
                
                if (isset($this->request->post['parent_txn_id'])) {
                    $parent_transaction = $this->model_payment_paypalexpress->getTransactionRow($this->request->post['parent_txn_id']);
                } else {
                    $parent_transaction = $this->model_payment_paypalexpress->getTransactionRowByReference($this->request->post['recurring_payment_id']);
                }
                
                if ($parent_transaction) {
                    if ($this->config->get('paypalexpress_debug') == 1) $this->log->write('Parent transaction exists');
                    ...
                    if (isset($this->request->post['payment_status']) && $this->request->post['payment_status'] == 'Refunded') {
                        $this->model_payment_expressipn->processRefund($this->request->post['mc_gross'], $parent_transaction['amount'], $parent_transaction['transaction_id']);
                    }
                    ...
                    if (isset($this->request->post['auth_status']) && $this->request->post['auth_status'] == 'Completed' && $parent_transaction['payment_status'] == 'Pending') {
                        ...
                        $this->model_payment_paypalexpress->updateOrder('Complete', $parent_transaction['order_id']);
                    }
                } else {
                    
                    //parent transaction doesn't exists, need to investigate?
                    if ($this->config->get('paypalexpress_debug') == 1) $this->log->write('Parent transaction not found');
                }
            }
            ...
            */
            ...
            $this->model_payment_paypalexpress->log(array('IPN was invalid'), 'IPN fail');
        } else {
            if ($this->config->get('paypalexpress_debug') == 1) {
    ...
    public function shipping() {
        ...
        $this->response->redirect($this->url->link('payment/paypalexpress/expressConfirm'));
    }
    ...
    public function recurringCancel() {
        ...
        $this->theme->model('account/recurring');
        $this->theme->model('payment/paypalexpress');
        $this->theme->language('account/recurring');
        
        $recur = $this->model_account_recurring->getProfile($this->request->get['recurring_id']);
        
        if ($recur && !empty($recur['reference'])) {
            
            $result = $this->model_payment_paypalexpress->recurringCancel($recur['reference']);
