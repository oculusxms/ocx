...
class Coupon extends Controller {
	...
	protected function getList() {
		...
		$data['pagination'] = $this->theme->paginate(
			...
			$this->url->link('sale/coupon', 'token=' . $this->session->data['token'] . $url . '&page={page}')
		);
	...
	public function history() {
		...
		$data['pagination'] = $this->theme->paginate(
			...
			10,
			$this->language->get('text_pagination'),
			$this->url->link('sale/coupon/history', 'token=' . $this->session->data['token'] . '&coupon_id=' . $this->request->get['coupon_id'] . '&page={page}', 'SSL')
		);	
