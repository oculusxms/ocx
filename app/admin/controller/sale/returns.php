...
class Returns extends Controller { 
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('sale/returns/info', 'token=' . $this->session->data['token'] . '&return_id=' . $result['return_id'] . $url, 'SSL')
			);

			$action[] = array(
				'text' => $this->language->get('text_edit'),
				'href' => $this->url->link('sale/returns/update', 'token=' . $this->session->data['token'] . '&return_id=' . $result['return_id'] . $url, 'SSL')
			);
		...
		$data['pagination'] = $this->theme->paginate(
			...
			$this->url->link('sale/returns', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL')
		);
	...
	public function history() {
		...
		$data['pagination'] = $this->theme->paginate(
			...
			10,
			$this->language->get('text_pagination'),
			$this->url->link('sale/returns/history', 'token=' . $this->session->data['token'] . '&return_id=' . $this->request->get['return_id'] . '&page={page}', 'SSL')
		);
