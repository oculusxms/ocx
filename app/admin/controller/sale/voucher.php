...
class Voucher extends Controller {
	...
	protected function getList() {
		...
		$data['pagination'] = $this->theme->paginate(
			...
			$this->url->link('sale/voucher', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL')
		);
	...
	public function history() {
		...
		$data['pagination'] = $this->theme->paginate(
			...
			10,
			$this->language->get('text_pagination'),
			$this->url->link('sale/voucher/history', 'token=' . $this->session->data['token'] . 
				'&voucher_id=' . $this->request->get['voucher_id'] . '&page={page}', 'SSL')
		);
