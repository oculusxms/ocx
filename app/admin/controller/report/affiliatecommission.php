...
class Affiliatecommission extends Controller {
    public function index() {
        ...
        foreach ($results as $result) {
            ...
            $data['affiliates'][] = array('affiliate' => $result['affiliate'], 'email' => $result['email'], 'status' => ($result['status'] ? $this->language->get('text_enabled') : $this->language->get('text_disabled')), 'commission' => $this->currency->format($result['commission'], $this->config->get('config_currency')), 'orders' => $result['orders'], 'total' => $this->currency->format($result['total'], $this->config->get('config_currency')), 'action' => $action);
        }
        ...
        $data['pagination'] = $this->theme->paginate($affiliate_total, $page, $this->config->get('config_admin_limit'), $this->language->get('text_pagination'), $this->url->link('report/affiliatecommission', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL'));
