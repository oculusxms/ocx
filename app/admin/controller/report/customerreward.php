...
class Customerreward extends Controller {
    public function index() {
        ...
        $this->breadcrumb->add('lang_heading_title', 'report/customerreward', $url);
        ...
        foreach ($results as $result) {
            ...
            $data['customers'][] = array('customer' => $result['customer'], 'email' => $result['email'], 'customer_group' => $result['customer_group'], 'status' => ($result['status'] ? $this->language->get('lang_text_enabled') : $this->language->get('lang_text_disabled')), 'points' => $result['points'], 'orders' => $result['orders'], 'total' => $this->currency->format($result['total'], $this->config->get('config_currency')), 'action' => $action);
        }
        ...
        $data['pagination'] = $this->theme->paginate($customer_total, $page, $this->config->get('config_admin_limit'), $this->language->get('lang_text_pagination'), $this->url->link('report/customerreward', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL'));
