...
class Saleorder extends Controller {
    public function index() {
        ...
        $this->breadcrumb->add('heading_title', 'report/saleorder', $url);
        ...
        foreach ($results as $result) {
            $data['orders'][] = array('date_start' => date($this->language->get('date_format_short'), strtotime($result['date_start'])), 'date_end' => date($this->language->get('date_format_short'), strtotime($result['date_end'])), 'orders' => $result['orders'], 'products' => $result['products'], 'tax' => $this->currency->format($result['tax'], $this->config->get('config_currency')), 'total' => $this->currency->format($result['total'], $this->config->get('config_currency')));
        }
        ...
        $data['pagination'] = $this->theme->paginate($order_total, $page, $this->config->get('config_admin_limit'), $this->language->get('text_pagination'), $this->url->link('report/saleorder', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL'));
