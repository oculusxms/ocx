...
class Saleshipping extends Controller {
    public function index() {
        $data = $this->theme->language('report/sale_shipping');
        ...
        $this->breadcrumb->add('lang_heading_title', 'report/saleshipping', $url);
        ...
        $data['pagination'] = $this->theme->paginate($order_total, $page, $this->config->get('config_admin_limit'), $this->language->get('lang_text_pagination'), $this->url->link('report/saleshipping', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL'));
        ...
        $this->response->setOutput($this->theme->view('report/sale_shipping', $data));
    }
}
