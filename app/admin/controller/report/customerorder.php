...
class Customerorder extends Controller {
    public function index() {
        ...
        $this->breadcrumb->add('heading_title', 'report/customerorder', $url);
        ...
        foreach ($results as $result) {
            ...
            $data['customers'][] = array('customer' => $result['customer'], 'email' => $result['email'], 'customer_group' => $result['customer_group'], 'status' => ($result['status'] ? $this->language->get('text_enabled') : $this->language->get('text_disabled')), 'orders' => $result['orders'], 'products' => $result['products'], 'total' => $this->currency->format($result['total'], $this->config->get('config_currency')), 'action' => $action);
        }
        ...
        $data['pagination'] = $this->theme->paginate($customer_total, $page, $this->config->get('config_admin_limit'), $this->language->get('text_pagination'), $this->url->link('report/customerorder', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL'));
