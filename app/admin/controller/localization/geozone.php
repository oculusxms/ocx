...
class Geozone extends Controller { 
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('localization/geozone', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$geo_zone_total = $this->model_localization_geozone->getTotalGeoZones();

		$results = $this->model_localization_geozone->getGeoZones($filter);
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('localization/geozone/update', 'token=' . $this->session->data['token'] . '&geo_zone_id=' . $result['geo_zone_id'] . $url, 'SSL')
			);
		...
		$data['sort_name'] = $this->url->link('localization/geozone', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_description'] = $this->url->link('localization/geozone', 'token=' . $this->session->data['token'] . '&sort=description' . $url, 'SSL');
		...
		$data['pagination'] = $this->theme->paginate(
			...
			$this->url->link('localization/geozone', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL')
		);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['geo_zone_id'])) {
			...
			$data['action'] = $this->url->link('localization/geozone/update', 'token=' . $this->session->data['token'] . '&geo_zone_id=' . $this->request->get['geo_zone_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('localization/geozone', 'token=' . $this->session->data['token'] . $url, 'SSL');

		if (isset($this->request->get['geo_zone_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$geo_zone_info = $this->model_localization_geozone->getGeoZone($this->request->get['geo_zone_id']);
		}
		...
		if (isset($this->request->post['zone_to_geo_zone'])) {
			...
			$data['zone_to_geo_zones'] = $this->model_localization_geozone->getZoneToGeoZones($this->request->get['geo_zone_id']);
		} else {
