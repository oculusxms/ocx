...
class Taxclass extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('localization/taxclass', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$tax_class_total = $this->model_localization_taxclass->getTotalTaxClasses();

		$results = $this->model_localization_taxclass->getTaxClasses($filter);
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('localization/taxclass/update', 'token=' . $this->session->data['token'] . '&tax_class_id=' . $result['tax_class_id'] . $url, 'SSL')
			);
		...
		$data['sort_title'] = $this->url->link('localization/taxclass', 'token=' . $this->session->data['token'] . '&sort=title' . $url, 'SSL');
		...
		$data['pagination'] = $this->theme->paginate(
			...
			$this->url->link('localization/taxclass', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL')
		);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['tax_class_id'])) {
			...
			$data['action'] = $this->url->link('localization/taxclass/update', 'token=' . $this->session->data['token'] . '&tax_class_id=' . $this->request->get['tax_class_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('localization/taxclass', 'token=' . $this->session->data['token'] . $url, 'SSL');

		if (isset($this->request->get['tax_class_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$tax_class_info = $this->model_localization_taxclass->getTaxClass($this->request->get['tax_class_id']);
		}
		...
		$data['tax_rates'] = $this->model_localization_taxrate->getTaxRates();

		if (isset($this->request->post['tax_rule'])) {
			$data['tax_rules'] = $this->request->post['tax_rule'];
		} elseif (isset($this->request->get['tax_class_id'])) {
			$data['tax_rules'] = $this->model_localization_taxclass->getTaxRules($this->request->get['tax_class_id']);
		} else {
