...
class Event extends Controller {
    ...
    public function get_wait_list() {
        ...
        if ($results) {
            foreach ($results as $result) {
                ...
                $data['attendees'][] = array('event_wait_list_id' => $result['event_wait_list_id'], 'event_id' => $result['event_id'], 'attendee' => $customer_info['firstname'] . ' ' . $customer_info['lastname'] . ' (' . $customer_info['username'] . ')', 'action' => $actions);
            }
        }
    ...
    public function autocomplete() {
        ...
        if (isset($this->request->get['name'])) {
            ...
            foreach ($results as $result) {
                $json[] = array('attendee_id' => $result['customer_id'], 'name' => strip_tags(html_entity_decode($result['username'], ENT_QUOTES, 'UTF-8')));
            }
        }
