...
class Event extends Controller { 
	...
	public function add_to_wait_list() {
  	...
  	public function insert_attendee() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateRoster()) {
			$exists = $this->model_catalog_event->checkAttendee($this->request->post);
			if (!$exists) {
				$this->model_catalog_event->addAttendee($this->request->post);
				$results = $this->model_catalog_event->getRoster($this->request->post['event_id']);
				$roster_html = '';
				if ($results) {
				...
				$json = array(
					'success'	=> 1,
					'message'	=> $this->language->get('text_add_s_success'),
					'roster'	=> $roster_html,
					'available'	=> $available
				);
			} else {
  	...
  	public function delete_attendee() {
		...
		if (isset($this->request->post['selected']) && $this->validateRoster()) {
			...
			foreach ($this->request->post['selected'] as $attendee_id) {
				$this->model_catalog_event->deleteAttendee($this->request->get['event_id'], $attendee_id);
				$a++;
	  		}
	...
	public function get_wait_list() {
		...
		if ($results) {
			foreach ($results as $result) {
				...
				$actions[] = array(
					...
					'href' => $this->url->link('catalog/event/add_to_event', 'token=' . $this->session->data['token'] . '&event_id=' . $result['event_id'] . '&customer_id=' . $result['customer_id'], 'SSL')
				);
				$actions[] = array(
					'text'	=> $this->language->get('text_remove'),
					'href'	=> $this->url->link('catalog/event/remove_from_list', 'token=' . $this->session->data['token'] . '&event_id=' . $result['event_id'] . '&event_wait_list_id=' . $result['event_wait_list_id'], 'SSL')
				);
		...
		$data['clear_list'] = $this->url->link('catalog/event/empty_wait_list', 'token=' . $this->session->data['token'] . '&event_id=' . $this->request->get['event_id'], 'SSL');
		...
		$this->response->setOutput($this->theme->view('catalog/waitlist', $data));
	}
	...
	public function empty_wait_list() {
		$this->theme->model('catalog/event');
		$this->model_catalog_event->emptyWaitList($this->request->get['event_id']);
		$this->response->redirect($this->url->link('catalog/event', 'token=' . $this->session->data['token'], 'SSL'));
	}
	...
	public function getList() {
		...
		if ($results) {
			foreach ($results as $result) {
				...
				$actions[] = array(
					...
					'href' => $this->url->link('catalog/event/update', 'token=' . $this->session->data['token'] . '&event_id=' . $result['event_id'], 'SSL')
				);
				
				$actions[] = array(
					'text'	=> $this->language->get('text_roster'),
					'href'	=> $this->url->link('catalog/event/roster', 'token=' . $this->session->data['token'] . '&event_id=' . $result['event_id'], 'SSL')
				);
				...
				$data['events'][] = array(
					...
					'date_time'		=> date($this->language->get('date_format_short') . ' ' . $this->language->get('time_format'), strtotime($result['date_time'])),
					'location'		=> $location,
					'cost'			=> $this->currency->format($result['cost']),
					'seats'			=> $result['seats'],
					'filled'		=> $result['seats'] - $result['filled'],
					'waitlist'		=> $this->model_catalog_event->getWaitListCount($result['event_id']),
					'waitlist_href'	=> $this->url->link('catalog/event/get_wait_list', 'token=' . $this->session->data['token'] . '&event_id=' . $result['event_id'], 'SSL'),
					'presenter'		=> $this->model_catalog_event->getPresenterName($result['presenter_id']),
		...
		$data['token'] 		 = $this->session->data['token'];
		$data['presenters']  = $this->url->link('catalog/event/presenter_list', 'token=' . $this->session->data['token'], 'SSL');
		$data['insert'] 	 = $this->url->link('catalog/event/insert', 'token=' . $this->session->data['token'], 'SSL');
		$data['delete'] 	 = $this->url->link('catalog/event/delete', 'token=' . $this->session->data['token'], 'SSL');

		$data = $this->theme->listen(__CLASS__, __FUNCTION__, $data);

		$data = $this->theme->render_controllers($data);
				
		$this->response->setOutput($this->theme->view('catalog/event_list', $data));
  	}
	...
	public function getForm() {
		...
		if (isset($this->error['hangout'])) {
			$data['error_hangout'] = $this->error['hangout'];
		} else {
			$data['error_hangout'] = '';
		}	
		...
		if (!isset($this->request->get['event_id'])) {
			...
			$data['action'] = $this->url->link('catalog/event/update', 'token=' . $this->session->data['token'] . '&event_id=' . $this->request->get['event_id'], 'SSL');
		}
		...
		if (!empty($event_info)) {
			if ($event_info['product_id'] == 0) {
				$data['product_id'] = $this->model_catalog_event->getProductId($event_info['event_id']);
			} else {
    	...
    	if (isset($this->request->post['hangout'])) {
      		$data['hangout'] = $this->request->post['hangout'];
    	} elseif (!empty($event_info)) {
			$data['hangout'] = $event_info['hangout'];
		} else {
      		$data['hangout'] = '';
    	}
		...
		$data['presenters'] = $this->model_catalog_event->getPresenters();

		$data = $this->theme->listen(__CLASS__, __FUNCTION__, $data);

		$data = $this->theme->render_controllers($data);
				
		$this->response->setOutput($this->theme->view('catalog/event_form', $data));
	}
	...
	public function presenter_list() {
		...
		$results = $this->model_catalog_event->getPresenters($filter);
		...
		if ($results) {
			foreach ($results as $result) {
				...
				$action[] = array(
					'text' => $this->language->get('text_edit'),
					'href' => $this->url->link('catalog/event/update_presenter', 'token=' . $this->session->data['token'] . '&presenter_id=' . $result['presenter_id'], 'SSL')
				);
		...
		$data['token']  = $this->session->data['token'];
		$data['insert'] = $this->url->link('catalog/event/insert_presenter', 'token=' . $this->session->data['token'], 'SSL');
		$data['delete'] = $this->url->link('catalog/event/delete_presenter', 'token=' . $this->session->data['token'], 'SSL');
		$data['cancel'] = $this->url->link('catalog/event', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->theme->view('catalog/presenter_list', $data));
  	}
	...
	public function presenter_form() {
		...
		if (!isset($this->request->get['presenter_id'])) {
			...
			$data['action'] = $this->url->link('catalog/event/update_presenter', 'token=' . $this->session->data['token'] . '&presenter_id=' . $this->request->get['presenter_id'], 'SSL');
		}
		
		$data['cancel'] = $this->url->link('catalog/event/presenter_list', 'token=' . $this->session->data['token'], 'SSL');

		if (isset($this->request->get['presenter_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      		$event_info = $this->model_catalog_event->getPresenter($this->request->get['presenter_id']);
    	}
		...
		$this->response->setOutput($this->theme->view('catalog/presenter_form', $data));
	}
	...
	public function roster() {
		...
		$results = $this->model_catalog_event->getRoster($event_id);
		
		if ($results) {
			foreach ($results as $result) {
				$data['attendees'][] = array(
					'attendee_id'	=> $result['attendee_id'],
					'name'			=> $this->model_catalog_event->getAttendeeName($result['attendee_id']),
					'date_added'	=> date($this->language->get('date_format_short'), $result['date_added'])
				);
			}
		}
		
		$data['event_name'] = $this->model_catalog_event->getEventName($event_id);
		$data['seats'] 		= $this->model_catalog_event->getSeats($event_id);
		$data['available'] 	= $this->model_catalog_event->getAvailable($event_id);
		
		$data['event_id'] 	= $event_id;
		
		$data['delete'] 	= $this->url->link('catalog/event/delete_attendee', 'token=' . $this->session->data['token'] . '&event_id=' . $event_id, 'SSL');
		$data['cancel'] 	= $this->url->link('catalog/event', 'token=' . $this->session->data['token'], 'SSL');
		$data['token'] 		= $this->session->data['token'];

		$data = $this->theme->listen(__CLASS__, __FUNCTION__, $data);

		$this->theme->loadjs('javascript/catalog/roster', $data);

		$data = $this->theme->render_controllers($data);
				
		$this->response->setOutput($this->theme->view('catalog/roster', $data));
	}
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['name'])) {
			...
			$filter = array(
				'filter_username' => $this->request->get['name'],
				'start'       => 0,
				'limit'       => 20
			);
