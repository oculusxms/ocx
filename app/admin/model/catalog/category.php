...
class Category extends Model {
	...
	public function deleteCategory($category_id) {
		$this->db->query("DELETE FROM {$this->db->prefix}category_path WHERE category_id = '" . (int)$category_id . "'");

		$query = $this->db->query("SELECT * FROM {$this->db->prefix}category_path WHERE path_id = '" . (int)$category_id . "'");

		foreach ($query->rows as $result) {	
			$this->deleteCategory($result['category_id']);
		}

		$this->db->query("DELETE FROM {$this->db->prefix}category WHERE category_id = '" . (int)$category_id . "'");
		$this->db->query("DELETE FROM {$this->db->prefix}category_description WHERE category_id = '" . (int)$category_id . "'");
		$this->db->query("DELETE FROM {$this->db->prefix}category_filter WHERE category_id = '" . (int)$category_id . "'");
		$this->db->query("DELETE FROM {$this->db->prefix}category_to_store WHERE category_id = '" . (int)$category_id . "'");
