...
class Vouchertheme extends Model {
	public function addVoucherTheme($data) {
		$this->db->query("
			INSERT INTO {$this->db->prefix}voucher_theme 
			SET 
		...
		foreach ($data['voucher_theme_description'] as $language_id => $value) {
			$this->db->query("
				INSERT INTO {$this->db->prefix}voucher_theme_description 
				SET 
		...
		$this->cache->delete('voucher');
	}

	public function editVoucherTheme($voucher_theme_id, $data) {
		$this->db->query("
			UPDATE {$this->db->prefix}voucher_theme 
			SET 
				image = '" . $this->db->escape(html_entity_decode($data['image'], ENT_QUOTES, 'UTF-8')) . "' 
			WHERE voucher_theme_id = '" . (int)$voucher_theme_id . "'
		");

		$this->db->query("DELETE FROM {$this->db->prefix}voucher_theme_description WHERE voucher_theme_id = '" . (int)$voucher_theme_id . "'");
	...
	public function deleteVoucherTheme($voucher_theme_id) {
		$this->db->query("DELETE FROM {$this->db->prefix}voucher_theme WHERE voucher_theme_id = '" . (int)$voucher_theme_id . "'");
		$this->db->query("DELETE FROM {$this->db->prefix}voucher_theme_description WHERE voucher_theme_id = '" . (int)$voucher_theme_id . "'");
	...
	public function getVoucherThemeDescriptions($voucher_theme_id) {
		...
		$query = $this->db->query("
			SELECT * 
			FROM {$this->db->prefix}voucher_theme_description 
			WHERE voucher_theme_id = '" . (int)$voucher_theme_id . "'
		");
	...
	public function getTotalVoucherThemes() {
		$query = $this->db->query("
			...
			FROM {$this->db->prefix}voucher_theme");
