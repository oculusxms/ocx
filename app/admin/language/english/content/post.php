<?php

/*
|--------------------------------------------------------------------------
|   Oculus XMS
|--------------------------------------------------------------------------
|
|   This file is part of the Oculus XMS Framework package.
|	
|	(c) Vince Kronlein <vince@ocx.io>
|	
|	For the full copyright and license information, please view the LICENSE
|	file that was distributed with this source code.
|	
*/

// Heading
$_['heading_title']          = 'Blog Posts';
$_['doc_title']              = 'Blog Posts';

// Text
$_['text_success']           = 'Success: You have modified posts!';
$_['text_default']           = 'Default';
$_['text_image_manager']     = 'Image Manager';
$_['text_browse']            = 'Browse Files';
$_['text_clear']             = 'Clear Image';
$_['text_build']             = 'Build Slug';
$_['text_posted']            = 'Posted';
$_['text_draft']             = 'Draft';

// Column
$_['column_name']            = 'Post Title';
$_['column_author']          = 'Author';
$_['column_category']        = 'Categories';
$_['column_status']          = 'Status';
$_['column_viewed']          = 'Views';
$_['column_date_added']      = 'Date Added';
$_['column_date_modified']   = 'Last Update';
$_['column_action']          = 'Action';

// Entry
$_['entry_name']             = 'Title:';
$_['entry_meta_keyword']     = 'Meta Tag Keywords:';
$_['entry_meta_description'] = 'Meta Tag Description:';
$_['entry_description']      = 'Description:';
$_['entry_store']            = 'Stores:';
$_['entry_author']           = 'Author:';
$_['entry_slug']             = 'URL Slug:<br /><span class="help">Do not use spaces instead replace spaces with - and make sure the slug is globally unique.</span>';
$_['entry_date_available']   = 'Publish Date:<span class="help">Post will be active on/after this date.</span>';
$_['entry_image']            = 'Image:';
$_['entry_status']           = 'Status:';
$_['entry_sort_order']       = 'Sort Order:';
$_['entry_category']         = 'Categories:';
$_['entry_related']          = 'Related Posts:<br /><span class="help">(Autocomplete)</span>';
$_['entry_tag']              = 'Post Tags:<br /><span class="help">comma separated</span>';
$_['entry_layout']           = 'Layout Override:';
$_['entry_visibility']       = 'Visibility:';

// Error
$_['error_warning']          = 'Warning: Please check the form carefully for errors.';
$_['error_permission']       = 'Warning: You do not have permission to modify posts.';
$_['error_name']             = 'Title must be greater than 3 and less than 255 characters.';
$_['error_description']      = 'Description must be greater than 3 characters.';
$_['error_slug']             = 'Warning: Slug is required for blog posts.';
$_['error_slug_found']       = 'ERROR: The slug %s is already in use, please set a different one in the input field.';
$_['error_name_first']       = 'ERROR: Please enter a name for your post before attempting to build a slug.';
