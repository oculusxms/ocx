	...
	<div class="col-sm-<?php $span = trim($column_left) ? 9 : 12; $span = trim($column_right) ? $span - 3 : $span; echo $span; ?>">
		...
		<div class="row">
			...
			<form id="product-info">
				<?php if ($price) { ?>
				<p class="price" title="<?= $text_price; ?>">
					<?php if (!$special) { ?>
					...
					<?php } ?>
					<?php if ($points) { ?>
						<i class="muted"><?= $text_points; ?> <?= $points; ?></i>
					<?php } ?>
				</p>
				<?php if ($tax && $tax != $price && $tax != $special) { ?>
					<p class="help"><?= $text_tax; ?> <?= $tax; ?></p>
				<?php } ?>
				<?php if ($discounts) { ?>
					<ul>
					<?php foreach ($discounts as $discount) { ?>
						<li><?= sprintf($text_discount, $discount['quantity'], $discount['price']); ?></li>
					<?php } ?>
					</ul>
				<?php } ?>
				<br>
				...
				<div class="form-group" id="option-<?= $option['product_option_id']; ?>">
					...
					<?php } elseif ($option['type'] == 'file') { ?>
						<input type="button" class="btn btn-default" value="<?= $button_upload; ?>" id="button-option-<?= $option['product_option_id']; ?>" class="btn btn-default"><input type="hidden" name="option[<?= $option['product_option_id']; ?>]" value="">
				...
				<div class="form-group">
					<label for="recurring_id"><span class="required">*</span> <?= $text_payment_recurring ?></label><br>
				...
				<div class="well well-lg">
					...
					<div class="form-inline">
						<label for="quantity"><?= $text_qty; ?></label>
						<input type="number" name="quantity" id="quantity" class="form-control" value="<?= $minimum; ?>" min="1" autocomplete="off">
						<button type="button" id="button-cart" class="btn btn-warning btn-lg"><?= $button_cart; ?> <i class="fa fa-shopping-cart"></i></button>
					</div>
					<div class="wishlist">
				...
				<?php else: ?>
				<?php if ($registered): ?>
				<div class="alert alert-info"><?= $text_already_registered; ?></div>
				<?php else: ?>
				<?php if (!$unavailable): ?>
				<?php if ($button_waitlist): ?>
				<div id="wait-response">
				<?= $text_unavailable_info; ?>
					<button type="button" value="" id="button-waitlist" class="btn btn-warning btn-lg">
						<i class="fa fa-sign-in"></i> <?= $button_waitlist; ?></button>
				</div>
				<?php elseif ($text_already_on): ?>
				<div class="alert alert-info"><?= $text_already_on; ?></div>
				<?php endif; ?>
				<?php else: ?>
				<div class="alert alert-danger"><?= $text_account_needed; ?></div>
				<?php endif; ?>
				<?php endif; ?>
				<?php endif; ?>
			</form>
			...
			<?php if ($review_status) { ?>
				...
				<div class="spacer">
					...
					&nbsp;&nbsp;&bull;&nbsp;&nbsp;
					<a onclick="$('a[href=\'#tab-review\']').click();$('html,body').animate({scrollTop:$('#write-review').offset().top - 100},'slow');"><?= $text_write; ?></a>
				</div>
			<?php } ?>
			</div>
		</div>
		<ul class="nav nav-tabs">
			<?php if (isset($event_name)): ?>
				<?php if ($tab_presenter): ?>
					<li class="active"><a href="#tab-event" data-toggle="tab"><?= $tab_event; ?></a></li>
			...
			<?php if (isset($event_name)): ?>
				<div id="tab-event" class="tab-pane active">
					<table class="table table-striped">
						...
						<?php endif; ?>
						<tr>
							<td class="col-sm-3"><?= $text_event_date; ?></td>
							<td><?= $event_date; ?></td>
						</tr>
						<tr>
							<td><?= $text_event_time; ?></td>
							<td><?= $event_time; ?></td>
						</tr>
						<tr>
							<td><?= $text_event_days; ?></td>
							<td>
			...
			<div id="tab-attribute" class="tab-pane">
				<table class="table table-striped">
					...
					<tbody>
						...
						<?php } ?>
						<tr>
							<td class="col-sm-3"><?= $text_model; ?></td>
							<td><?= $model; ?></td>
						</tr>
						<?php if ($reward) { ?>
							<tr>
				...
				<div id="tab-review" class="tab-pane">
					...
					<fieldset id="write-review">
						...
						<form id="review-form" class="form-horizontal">
							<div class="form-group">
								<label class="control-label col-sm-3"><?= $entry_rating; ?></label>
								<div class="col-sm-6">
							...
							<div class="form-group">
								...
								<div class="col-sm-6">
									<input type="text" name="name" value="" class="form-control" placeholder="<?= $entry_name; ?>"  id="name">
								</div>
							</div>
							<div class="form-group">
								<label class="control-label col-sm-3" for="text"><?= $entry_review; ?></label>
								<div class="col-sm-6">
									<textarea name="text" class="form-control" placeholder="<?= $entry_review; ?>"  rows="4" id="text" spellcheck="false"></textarea>
							...
							<div class="form-actions">
								<div class="form-actions-inner text-right">
									<button type="button" id="button-review" onclick="addReview(<?= $product_id; ?>,this);" class="btn btn-primary"><?= $button_continue; ?></button>
								</div>
							</div>
						</form>
						<?php else: ?>
						<div class="alert alert-danger"><?= $text_please_login; ?></div>
						<?php endif; ?>
					</fieldset>
				</div>
			<?php } ?>
		...
		<?php if ($products) { ?>
			<hr>
			<h4 class="text-muted"><?= $tab_related; ?></h4>
			<?php 
				$class_prefix 	= '';
