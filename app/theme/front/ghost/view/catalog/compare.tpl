					...
					<tr>
						...
						<?php foreach ($products as $product) { ?>
							<td class="text-center">
								<a class="btn btn-danger" href="<?= $product['remove']; ?>"><i class="fa fa-trash-o"></i><span class="hidden-xs"> <?= $button_remove; ?></span></a>
								<?php if ($product['event_id'] == 0): ?>
								<button type="button" 
									data-cart="<?= $product['product_id']; ?>" 
									class="btn btn-primary"><?= str_replace('Cart', '<i title="Cart" class="fa fa-shopping-cart"></i>', $button_cart); ?>
								</button>
								<?php else: ?>
								<button type="button" 
									data-cart="<?= $product['product_id']; ?>" 
									data-event="<?= $product['event_id']; ?>" 
									class="btn btn-primary"><?= str_replace('Event', '<i title="View Event" class="fa fa-users"></i>', $button_view_event); ?>
								<?php endif; ?>
							</td>
						<?php } ?>
					</tr>
