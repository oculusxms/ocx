<p class="heading1" align="<?= $text_align; ?>" style="-ms-text-size-adjust:100%; mso-line-height-rule:exactly; font-family:Helvetica, Arial, sans-serif; font-size:18px; line-height:22px; color:<?= $body_font_color; ?>; margin-top:0px; margin-bottom:10px;">
	<strong><?= $title; ?></strong>
</p>
...
<thead>
	<tr>
    	<th colspan="2" style="font-size:14px; padding:6px 5px 4px; background:#ededed; font-weight:bold;"><?= $text_return_detail; ?></th>
   	</tr>
</thead>
<tbody>
	<tr>
    	<td style="font-size:12px; line-height:18px; padding:4px 5px; border-bottom:1px solid #e0e0e0; background:#fafafa;">
          	<b><?= $text_firstname; ?></b> <?= $firstname; ?>
          	<br /><b><?= $text_lastname; ?></b> <?= $lastname; ?>
			<?php if($email){ ?><br /><b><?= $text_email; ?></b> <a href="mailto:<?= $email; ?>" style="color:<?= $body_link_color; ?>; text-decoration:none; word-wrap:break-word;"><?= $email; ?></a><?php } ?>
			<?php if($telephone){ ?><br /><b><?= $text_telephone; ?></b> <?= $telephone; ?><?php } ?>
        </td>
        <td style="font-size:12px; line-height:18px; padding:4px 5px; border-bottom:1px solid #e0e0e0; border-left:1px solid #e0e0e0; background:#fafafa;">
        	<b><?= $text_order_id; ?></b> <?= $order_id; ?>
          	<?php if($order_date){ ?><br /><b><?= $text_order_date; ?></b> <?= $order_date; ?><?php } ?>
        </td>
	</tr>
</tbody>
</table>
...
<p class="link" align="<?= $text_align; ?>" style="-ms-text-size-adjust:100%; mso-line-height-rule:exactly; font-family:Helvetica, Arial, sans-serif; font-size:12px; line-height:18px; color:<?= $body_font_color; ?>; margin-top:5px; margin-bottom:0px;">
	<?= $text_action; ?>:<br />
