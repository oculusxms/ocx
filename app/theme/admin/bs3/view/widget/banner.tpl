		...
		<form class="form-inline" action="<?= $action; ?>" method="post" enctype="multipart/form-data" id="form">
			<table id="widget" class="table table-bordered table-striped">
				<thead>
					<tr>
						<th><?= $entry_banner; ?></th>
						<th><b class="required">*</b> <?= $entry_dimension; ?></th>
						<th><?= $entry_layout; ?></th>
				...
				<?php foreach ($widgets as $widget) { ?>
					<tr id="widget-row<?= $widget_row; ?>">
						<td><select name="banner_widget[<?= $widget_row; ?>][banner_id]" class="form-control">
						...
						<td><input type="text" name="banner_widget[<?= $widget_row; ?>][width]" value="<?= $widget['width']; ?>" class="form-control">
							<input type="text" name="banner_widget[<?= $widget_row; ?>][height]" value="<?= $widget['height']; ?>" class="form-control">
							<?php if (isset($error_dimension[$widget_row])) { ?>
							<div class="text-danger"><?= $error_dimension[$widget_row]; ?></div>
							<?php } ?></td>
						<td><select name="banner_widget[<?= $widget_row; ?>][layout_id]" class="form-control">
						...
						<td><select name="banner_widget[<?= $widget_row; ?>][position]" class="form-control">
						...
						<td><div class="btn-group" data-toggle="buttons">
							...
							<label class="btn btn-default active" title="<?= $text_enabled; ?>">
								<input type="radio" name="banner_widget[<?= $widget_row; ?>][status]" value="1" checked=""><i class="fa fa-play"></i></label>
							<label class="btn btn-default" title="<?= $text_disabled; ?>">
								<input type="radio" name="banner_widget[<?= $widget_row; ?>][status]" value="0"><i class="fa fa-pause"></i></label>
							<?php } else { ?>
							<label class="btn btn-default" title="<?= $text_enabled; ?>">
								<input type="radio" name="banner_widget[<?= $widget_row; ?>][status]" value="1"><i class="fa fa-play"></i></label>
							<label class="btn btn-default active" title="<?= $text_disabled; ?>">
								<input type="radio" name="banner_widget[<?= $widget_row; ?>][status]" value="0" checked=""><i class="fa fa-pause"></i></label>
							<?php } ?>
						</div></td>
						<td class="text-right">
							<input type="text" name="banner_widget[<?= $widget_row; ?>][sort_order]" value="<?= $widget['sort_order']; ?>" class="form-control">
						</td>
						<td>
