...
<div class="panel panel-default">
	<div class="panel-heading">
		<div class="clearfix">
			<div class="pull-left h2"><i class="hidden-xs fa fa-cogs"></i><?= $heading_title; ?></div>
	...
	<div class="panel-body">
		<form class="form-horizontal" action="<?= $action; ?>" method="post" enctype="multipart/form-data" id="form">
				...
				<div class="control-field col-sm-4">
					<select name="type" class="form-control">
						<optgroup label="<?= $text_choose; ?>">
							...
							<option value="image"<?= ($type == 'image') ? ' selected' : ''; ?>><?= $text_image; ?></option>
						</optgroup>
			...
			<table id="option-value" class="table table-bordered table-striped">
				<thead>
					<tr>
						<th class="col-sm-6"><b class="required">*</b> <?= $entry_option_value; ?></th>
						<th><?= $entry_image; ?></th>
					...
					<tr id="option-value-row<?= $option_value_row; ?>">
						...
						<td><div class="media">
							...
							<div class="media-body hidden-xs">
								<a class="btn btn-default" onclick="image_upload('image<?= $option_value_row; ?>','thumb<?= $option_value_row; ?>');"><?= $text_browse; ?></a>
								<a class="btn btn-default" onclick="$('#thumb<?= $option_value_row; ?>').attr('src', '<?= $no_image; ?>'); $('#image<?= $option_value_row; ?>').val('');"><?= $text_clear; ?></a>
							</div>
						</div></td>
				...
				<tfoot>
					<tr>
						<td colspan="3"></td>
						<td><a onclick="addOptionValue();" class="btn btn-info"><i class="fa fa-plus-circle"></i><span class="hidden-xs"> <?= $button_add_option_value; ?></span></a></td>
					</tr>
				</tfoot>
			</table>
		</form>
