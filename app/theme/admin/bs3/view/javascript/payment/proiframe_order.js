	...
	function capture(){
		...
		if(amt == '' || amt == 0){
			...
			$.ajax({
				...
				url:'index.php?route=payment/proiframe/capture&token=<?= $token; ?>',
				beforeSend:function(){
					$('#button-capture').hide();
				...
				success:function(data){
					...
					if(data.error==true){
						...
						if (data.failed_transaction) {
							...
							html += '<td><a onclick="resendTransaction(this); return false;" href="<?= $resend_link ?>&paypal_iframe_order_transaction_id='+data.failed_transaction.paypal_iframe_order_transaction_id +'"><?= $text_resend ?></a></td>';
							html += '/<tr>';
							$('#paypal_transactions').append(html);
						}
					}
	...
	function doVoid(){
		if (confirm('<?= $text_confirm_void; ?>')) {
			$.ajax({
				...
				url:'index.php?route=payment/proiframe/void&token=<?= $token ?>',
				beforeSend:function(){
					$('#button-void').hide();
...
function reauthorise() {
	$.ajax({
		...
		dataType:'json',
		data: {'order_id':<?= $order_id ?> },
		url:'index.php?route=payment/proiframe/reauthorise&token=<?= $token; ?>',
		beforeSend:function(){
			$('#button-reauthorise').hide();
			$('.icon-loading').remove();
			$('#button-reauthorise').after('<img src="<?= HTTPS_SERVER; ?>asset/default/img/loading.gif" class="loading">');
		},
		success:function(data){
			$('.icon-loading').remove();
