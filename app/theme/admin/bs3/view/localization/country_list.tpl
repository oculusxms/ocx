			...
			<table class="table table-bordered table-striped table-hover">
				<thead>
					<tr>
						...
						<th class="hidden-xs"><a href="<?= $sort_iso_code_2; ?>"><?= $column_iso_code_2; echo ($sort == 'iso_code_2') ? '<i class="caret caret-' . strtolower($order) . '"></i>' : ''; ?></a></th>
						<th class="hidden-xs"><a href="<?= $sort_iso_code_3; ?>"><?= $column_iso_code_3; echo ($sort == 'iso_code_3') ? '<i class="caret caret-' . strtolower($order) . '"></i>' : ''; ?></a></th>
				...
				<tbody data-link="row" class="rowlink">
					...
					<tr>
						<td class="rowlink-skip text-center"><?php if ($country['selected']) { ?>
							<input type="checkbox" name="selected[]" value="<?= $country['country_id']; ?>" checked="">
							<?php } else { ?>
							<input type="checkbox" name="selected[]" value="<?= $country['country_id']; ?>">
							<?php } ?></td>
						<td><?= $country['name']; ?></td>
						<td class="hidden-xs"><?= $country['iso_code_2']; ?></td>
						<td class="hidden-xs"><?= $country['iso_code_3']; ?></td>
