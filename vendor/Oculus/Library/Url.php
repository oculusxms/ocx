...
class Url extends LibraryService {
	...
	public function link($route, $args = '', $secure = false) {
		...
		if ($args):
			if (parent::$app['active.fascade'] === FRONT_FASCADE):  // Catalog access only
				$cache = parent::$app['cache'];
				parse_str($args, $urlargs);
				if (array_key_exists('product_id', $urlargs)): // Product URLS
				...
				elseif (array_key_exists('post_id', $urlargs)): // Blog post URLS
					if (!parent::$app['config_top_level']):
					...
					if (parent::$app['config_top_level']):
						if (array_key_exists('path', $urlargs)): // Product categories
							$paths = explode('_', $urlargs['path']);
							$urlargs['path'] = array_pop($paths);
						endif;
						if (array_key_exists('bpath', $urlargs)): // Blog categories
		...
		if (parent::$app['active.fascade'] === FRONT_FASCADE):  // Catalog access 
			return $this->rewrite($url);
		elseif (parent::$app['active.fascade'] === INSTALL_FASCADE): // Installer access
	...
	*/
	private function rewrite($link) {
				...
				if (isset($route['route'])):
					switch($route['route']):
						case $value:
							$url .= '/' . $key;
							break;
						case 'account/returns/insert':  // special case for return insert
		...
		if ($url):
			...
			return $url_info['scheme'] . '://' . $url_info['host'] . (isset($url_info['port']) ? 
				':' . $url_info['port'] : '') . str_replace('/index.php', '', $url_info['path']) . $url . $query;
		else:
			if (parent::$app['theme']->style === 'shop'):
	...
	*/
	...
	public function build_slug ($string) {
		...
		$string = str_replace (' ', '-', $string);
		$string = str_replace ('--', '-', $string);
		$string = str_replace ('-------', '-', $string); // hack for rocks off f
