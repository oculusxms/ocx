...
$(function(){
	...
	$(document).on('click', '#event-slug-btn', function(){
		...
		$url = 'index.php?route=catalog/event/slug&token='+token+'&name='+encodeURIComponent(name);
		if (event_id) {
			...
			success:function(json){
				...
				if(json['order_product']!=''){
					...
					for(i=0;i<json['order_product'].length;i++){
						...
						html += '<td class="text-right">'+product['price'].toFixed(2)+'<input type="hidden" name="order_product['+product_row+'][price]" value="'+product['price']+'"></td>';
						html += '<td class="text-right">'+product['total'].toFixed(2)+'<input type="hidden" name="order_product['+product_row+'][total]" value="'+product['total']+'"><input type="hidden" name="order_product['+product_row+'][tax]" value="'+product['tax']+'"><input type="hidden" name="order_product['+product_row+'][reward]" value="'+product['reward']+'"></td>';
						html += '</tr>';
						product_row++;		
					}
					$('#product').html(html);
				...
				if(json['order_giftcard']!=''){
					...
					for(i in json['order_giftcard']){
						...
						html += '<td class="text-right">'+giftcard['amount'].toFixed(2)+'</td>';
						html += '<td class="text-right">'+giftcard['amount'].toFixed(2)+'</td>';
						html += '</tr>';
						giftcard_row++;
					}
				...
				if(json['order_product']!=''||json['order_giftcard']!=''||json['order_total']!=''){
					...
					if(json['order_product']!=''){
						for(i=0;i<json['order_product'].length;i++){
							...
							html += '<td class="text-right">'+product['price'].toFixed(2)+'</td>';
							html += '<td class="text-right">'+product['total'].toFixed(2)+'</td>';
							html += '</tr>';
						}			
					}
					if(json['order_giftcard']!=''){
					...
					for(i in json['order_total']){
						...
						html += '<td class="text-right">'+total['value'].toFixed(2)+'</td>';
						html += '</tr>';
						total_row++;
					}
					$('#total').html(html);
